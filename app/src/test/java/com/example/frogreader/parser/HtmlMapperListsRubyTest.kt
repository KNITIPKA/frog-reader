...
class HtmlMapperListsRubyTest {
    ...
    fun `floated image travels with its paragraph instead of a block element`() {
        ...
        val elements = mapper.map(
            Jsoup.parse(
                "<p><img class='small' src='pic.png'/>Текст, который обтекает картинку слева.</p>" +
        ...
        // The floated image is NOT duplicated as a block image; the plain
        // one still is.
        val images = elements.filterIsInstance<ContentElement.Image>().map { it.path }
        assertEquals(listOf("/resolved/big.png"), images)
    }
