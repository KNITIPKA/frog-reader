...
class GeneratedContentTest {
    ...
    @Test
    fun `svg wrapped raster preserves the svg viewport and child image`() {
        var captured: String? = null
        val mapper = HtmlMapper(
            resolveImage = { src -> "/raster/$src" },
            resolveInlineSvg = { markup ->
                captured = markup
                "/rendered/wrapper.svg"
            },
        )
        val elements = mapper.map(
            Jsoup.parse(
        ...
        val image = elements.filterIsInstance<ContentElement.Image>().single()
        assertEquals("/rendered/wrapper.svg", image.path)
        assertTrue(captured!!.contains("viewBox=\"0 0 10 10\""))
        assertTrue(captured!!.contains("<image"))
        assertTrue(captured!!.contains("cover.jpg"))
    }
