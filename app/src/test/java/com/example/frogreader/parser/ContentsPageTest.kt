...
class ContentsPageTest {
    ...
    @Test
    fun `footnote links still open notes instead of navigating`() {
        val file = book()
        assumeTrue(file.exists() && file.canRead())

        val content = EpubParser.parseContent(file, tempFolder.newFolder())
        // The book's endnotes are anchor links and must stay footnotes.
        assertTrue("notes: ${content.notes.size}", content.notes.size > 100)
        assertTrue(content.notes.keys.all { '#' in it })
    }
}
