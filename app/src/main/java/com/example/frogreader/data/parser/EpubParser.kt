...
object EpubParser {
     ...
     * With a fragment it names an anchor (footnote); without one it names a
     * whole document — a Contents entry that should jump there.
     */
    private fun resolveLinkKey(href: String, chapterPath: String, chapterDir: String): String? {
        ...
        if (fragment.isEmpty()) {
            ...
            if (!path.substringAfterLast('.').lowercase().startsWith("htm") &&
                !path.substringAfterLast('.').lowercase().startsWith("xhtm")
            ) {
    ...
    private fun spineItems(opf: Document, items: List<ManifestItem>): List<ManifestItem> {
        ...
        val fromSpine = opf.select("spine > itemref")
            ...
            .mapNotNull { itemsById[it.attr("idref")] }
        if (fromSpine.isNotEmpty()) return fromSpine
        return items.filter { it.mediaType.contains("html", ignoreCase = true) }
    }
    ...
    private fun cssResolverFor(
        ...
        for (link in doc.select("link[href]")) {
            ...
            if (!rel.equals("stylesheet", true) && !type.contains("css", true)) continue
            ...
            if (text != null) {
                keys += path
                sheets += CssResolver.Sheet(text, path.substringBeforeLast('/', ""))
            }
        }
        for (style in doc.select("style")) {
            ...
            if (text.isNotBlank()) {
                ...
                sheets += CssResolver.Sheet(text, chapterDir)
            }
        }
    ...
    private fun chapterTitles(
        ...
        items.firstOrNull { it.properties.contains("nav") }?.let { navItem ->
            ...
            zipEntry(zip, navPath)?.let { entry ->
                ...
                val toc = doc.getElementsByAttributeValue("epub:type", "toc").firstOrNull()
                    ?: doc.selectFirst("nav")
        ...
        if (titles.isEmpty()) {
            val ncxItem = items.firstOrNull { it.mediaType == "application/x-dtbncx+xml" }
            if (ncxItem != null) {
    ...
    private fun zipEntry(zip: ZipFile, path: String): ZipEntry? {
        ...
        val decoded = runCatching { URLDecoder.decode(path, "UTF-8") }.getOrNull()
    ...
    private fun resolvePath(baseDir: String, href: String): String {
        val raw = runCatching { URLDecoder.decode(href, "UTF-8") }.getOrDefault(href)
